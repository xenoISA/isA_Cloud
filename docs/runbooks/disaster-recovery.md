...
3. If data is corrupted, restore from backup:
   ```bash
   # Restores all services from the backup directory
...
1. Verify PVCs are intact:
   ...
   kubectl get pvc -n isa-cloud-local
   ```
2. Re-deploy all services via Helm (using local values files):
   ```bash
   # Re-install infrastructure (values in deployments/kubernetes/local/values/)
   helm upgrade --install postgresql bitnami/postgresql -n isa-cloud-local \
     -f deployments/kubernetes/staging/values/postgresql.yaml
   helm upgrade --install redis bitnami/redis -n isa-cloud-local \
     -f deployments/kubernetes/staging/values/redis.yaml
   helm upgrade --install neo4j neo4j/neo4j -n isa-cloud-local \
     -f deployments/kubernetes/local/values/neo4j.yaml
   helm upgrade --install qdrant qdrant/qdrant -n isa-cloud-local \
     -f deployments/kubernetes/local/values/qdrant.yaml
   helm upgrade --install consul hashicorp/consul -n isa-cloud-local \
     -f deployments/kubernetes/local/values/consul.yaml
   helm upgrade --install nats nats/nats -n isa-cloud-local \
     -f deployments/kubernetes/local/values/nats.yaml
   helm upgrade --install apisix apisix/apisix -n isa-cloud-local \
     -f deployments/kubernetes/local/values/apisix.yaml
...
```bash
# For local dev (KIND) — use the project's kind config
kind create cluster --name isa-cloud-local \
  --config deployments/kubernetes/local/kind-config.yaml
...
# Values files: deployments/kubernetes/local/values/ (local dev)
#               deployments/kubernetes/staging/values/ (staging)
helm install postgresql bitnami/postgresql -n isa-cloud-local \
  -f deployments/kubernetes/staging/values/postgresql.yaml
helm install redis bitnami/redis -n isa-cloud-local \
  -f deployments/kubernetes/staging/values/redis.yaml
helm install consul hashicorp/consul -n isa-cloud-local \
  -f deployments/kubernetes/local/values/consul.yaml
helm install nats nats/nats -n isa-cloud-local \
  -f deployments/kubernetes/local/values/nats.yaml
helm install neo4j neo4j/neo4j -n isa-cloud-local \
  -f deployments/kubernetes/local/values/neo4j.yaml
helm install qdrant qdrant/qdrant -n isa-cloud-local \
  -f deployments/kubernetes/local/values/qdrant.yaml
helm install minio minio/minio -n isa-cloud-local
helm install apisix apisix/apisix -n isa-cloud-local \
  -f deployments/kubernetes/local/values/apisix.yaml
```
...
# The script restores: PostgreSQL, MinIO, Qdrant, Neo4j, Redis, NATS, Consul, MQTT
./scripts/restore-cluster-data.sh ./backups/cluster-backup-YYYYMMDD_HHMMSS
```
...
```bash
# APISIX routes are restored as part of the full restore script.
# Verify routes
