...
HMAC-SHA256 signature (the #374 contract, exercised through the stub
``verify_telemetry_hmac`` backed by an env-resolved secret).
...
from fleet_console.telemetry_credential import _ENV_PREFIX, compute_telemetry_hmac
...
def seeded_ledger(engine):
    """Seed one current ledger row the telemetry can tie back to."""
...
def telemetry_secret(monkeypatch):
    """Register the deployment secret in the stub credential store (env-backed)."""
    monkeypatch.setenv(_ENV_PREFIX + DEPLOYMENT_SECRET_ID, SECRET.decode())
...
def _signed_headers(raw: bytes, *, secret: bytes = SECRET, secret_id: str = DEPLOYMENT_SECRET_ID):
    return {
        "X-Deployment-Secret-Id": secret_id,
        "X-Telemetry-Signature": compute_telemetry_hmac(secret, raw),
    }
