...
func (c *IgniteClient) ListVMs(ctx context.Context, labelFilters map[string]string) ([]*VMStatus, error) {
	...
	vms := make([]*VMStatus, 0, len(vmsData))
	for _, vmData := range vmsData {
		...
		// TODO: Apply label filters based on labelFilters map
