// Qdrant gRPC 服务端主程序
// 文件名: cmd/qdrant-service/main.go
package main
...
import (
	...
	"github.com/isa-cloud/isa_cloud/cmd/qdrant-service/server"
...
func loadConfig() *Config {
	return &Config{
		Qdrant: QdrantConfig{
			...
			UseSSL: getEnv("QDRANT_USE_SSL", "false") == "true",
		},
		Consul: ConsulConfig{
...
func main() {
	...
	qdrantConfig := &qdrant.Config{
		...
		UseSSL: cfg.Qdrant.UseSSL,
	}

	qdrantClient, err := qdrant.NewClient(ctx, qdrantConfig)
	if err != nil {
	...
	// 创建 Qdrant gRPC 服务
	qdrantServer := server.NewQdrantServer(qdrantClient)
	pb.RegisterQdrantServiceServer(grpcServer, qdrantServer)
	...
	log.Printf("Qdrant gRPC Service listening on :%d", cfg.GRPCPort)
	...
	// 优雅关闭处理
	go handleShutdown(grpcServer, consulClient, serviceID, qdrantClient)
...
func registerConsul(cfg *Config) (*consulapi.Client, string, error) {
	...
	if serviceHostname == "" {
		...
		if err != nil {
			serviceHostname = "isa-qdrant-grpc"
		} else {
	...
	registration := &consulapi.AgentServiceRegistration{
		ID:      serviceID,
		Name:    "qdrant-grpc-service",
		Address: serviceHostname,
		Port:    cfg.GRPCPort,
		Tags:    []string{"grpc", "qdrant", "vector-database"},
...
func handleShutdown(grpcServer *grpc.Server, consulClient *consulapi.Client, serviceID string, qdrantClient *qdrant.Client) {
