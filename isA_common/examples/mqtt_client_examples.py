...
def example_06_qos_levels(host="localhost", port=50053):
    ...
    with MQTTClient(host=host, port=port, user_id="example-user") as client:
        ...
        if conn:
            ...
            print("   Use for: Non-critical logs, telemetry")

            # QoS 1: At least once
            print("\n📡 QoS 1 (At Least Once):")
            client.publish(session_id, "sensors/data", b"Sensor reading", qos=1)
            print("   Use for: Sensor data, analytics")

            # QoS 2: Exactly once
            print("\n📡 QoS 2 (Exactly Once):")
            client.publish(session_id, "commands/critical", b"Execute shutdown", qos=2)
            print("   Use for: Critical commands, financial transactions")
...
def example_07_retained_messages(host="localhost", port=50053):
    ...
    with MQTTClient(host=host, port=port, user_id="example-user") as client:
        ...
        if conn:
            ...
            payload = json.dumps(
                {"temperature": 21.5, "mode": "heat", "timestamp": datetime.now().isoformat()}
            ).encode()

            success = client.set_retained_message(topic, payload, qos=1)
            if success:
                print(f"\n💾 Retained message set on '{topic}'")
                print("   New subscribers will receive this as last known state")
    ...
    with MQTTClient(host=host, port=port, user_id="example-user") as client:
        ...
        devices = [
            (
                ...
                "sensor",
                {"location": "Living Room", "model": "DHT22"},
            ),
            (
                "sensor-temp-02",
                "Temperature Sensor 2",
                "sensor",
                {"location": "Bedroom", "model": "DHT22"},
            ),
            (
                "gateway-01",
                "Main Gateway",
                "gateway",
                {"location": "Server Room", "firmware": "v2.1.0"},
            ),
        ]
    ...
    with MQTTClient(host=host, port=port, user_id="example-user") as client:
        ...
        if conn:
            ...
            if result:
                ...
                print("   Topic: sensors/binary/raw")
        ...
        if conn:
            ...
            for i, temp in enumerate(readings, 1):
                ...
                if temp > threshold_high:
                    ...
                    client.publish_json(
                        session_id, "alerts/temperature/high", alert, qos=2, retained=True
                    )
                    ...
                    client.publish_json(
                        session_id, "alerts/temperature/low", alert, qos=2, retained=True
                    )
    ...
    with MQTTClient(host=host, port=port, user_id="example-user") as client:
        ...
        if conn:
            ...
            for device in devices:
                ...
                client.publish_json(
                    session_id, f"devices/{device}/status", status, qos=1, retained=True
                )
