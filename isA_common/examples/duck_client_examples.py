...
def example_01_health_check(host="localhost", port=50052):
    ...
    with DuckDBClient(host=host, port=port, user_id="example_user") as client:
        ...
        if health:
            ...
            print("   Ready for analytical computing")
            print("   MinIO integration enabled for cold storage")
        else:
...
def example_02_database_management(host="localhost", port=50052):
    ...
    with DuckDBClient(host=host, port=port, user_id="example_user") as client:
        ...
        if db_info:
            ...
            print("   Database deleted (including MinIO files)")
...
def example_03_table_operations(host="localhost", port=50052):
    ...
    with DuckDBClient(host=host, port=port, user_id="example_user") as client:
        ...
        try:
            ...
            if success:
                print("✅ Table 'user_events' created")
...
def example_06_import_from_minio(host="localhost", port=50052):
    ...
    with (
        DuckDBClient(host=host, port=port, user_id="example_user") as duckdb,
...
"""
            ...
            print("   ✅ Cold data stored in MinIO: duckdb-data/cold-data/orders.csv")
            ...
            if success:
                ...
                for row in results:
                    print(
                        f"      {row['product_id']}: {row['total_quantity']} units, ${row['total_revenue']:.2f} revenue"
                    )
...
"""
            ...
            minio.put_object(
                "duckdb-data", "logs/app.csv", io.BytesIO(csv_data.encode()), len(csv_data.encode())
            )

            print("\n⚡ Zero-Copy Query: Analyzing cold data directly from MinIO...")
            print("   (No data import required!)")

            # Query MinIO file directly - no import needed!
            result = duckdb.query_minio_file(
                database_id, "duckdb-data", "logs/app.csv", file_format="csv", limit=100
            )

            if result:
                print(f"\n✅ Query successful! Retrieved {len(result)} rows")
                print("   (Data never left MinIO - true zero-copy analytics)")
...
def example_08_export_to_minio(host="localhost", port=50052):
    ...
    with (
        ...
        try:
            ...
            print("   ✅ 6 days of sales data loaded")
...
def example_09_advanced_analytics(host="localhost", port=50052):
    ...
    with DuckDBClient(host=host, port=port, user_id="example_user") as client:
        ...
        try:
            ...
            result = client.execute_query(database_id, sql)
            print("   Top sales by product category:")
            for row in result[:4]:
                print(
                    f"      {row['product']}: ${row['amount']} (rank #{row['rank_in_category']}, avg: ${row['category_avg']:.2f})"
                )
            ...
            for row in result:
                print(
                    f"      {row['product']}: {row['high_value_count']} sales, avg ${row['avg_amount']:.2f}"
                )

            print("\n💡 DuckDB excels at:")
            print("   - Window functions (ROW_NUMBER, RANK, LAG, LEAD)")
            print("   - CTEs for readable complex queries")
            print("   - OLAP operations (aggregations, analytics)")
...
def example_10_hot_data_lifecycle(host="localhost", port=50052):
    ...
    with DuckDBClient(host=host, port=port, user_id="example_user") as client:
        ...
        try:
            ...
            print("      ✅ Created and populated temp_sessions")

            # Use hot data
            print("\n   Step 2: USE hot data for fast analytics")
            result = client.execute_query(
                database_id, "SELECT AVG(events) as avg_events FROM temp_sessions"
            )
...
def example_11_polars_duckdb_pattern(host="localhost", port=50052):
    ...
    with DuckDBClient(host=host, port=port, user_id="example_user") as client:
        ...
        try:
            ...
            print("      # In production, pass result to Polars:")
            print("      import polars as pl")
            print("      df = pl.DataFrame(result)")
            print("      df = df.with_columns([")
            print("          (pl.col('total_amount') * 0.1).alias('commission'),")
            print("          (pl.col('total_amount') * 1.08).alias('with_tax')")
            print("      ])")
            ...
            for row in result:
                print(
                    f"      User {row['user_id']}: ${row['total_amount']:.2f} "
                    + f"(commission: ${row['commission']:.2f}, with tax: ${row['with_tax']:.2f})"
                )

            print("\n💡 Best of both worlds:")
            print("   - DuckDB: SQL queries, filtering, aggregation")
            print("   - Polars: DataFrame operations, transformations, ML features")
...
def example_12_data_warehouse_pattern(host="localhost", port=50052):
    ...
    with DuckDBClient(host=host, port=port, user_id="example_user") as client:
        ...
        try:
            ...
            print("   - Dimension tables: Reference data (products, customers)")
            print("   - Fact tables: Transactional data (sales, orders)")
            print("   - DuckDB: Fast OLAP queries on star schema")
    ...
    with DuckDBClient(host=host, port=port, user_id="example_user") as client:
        ...
        try:
            ...
            for row in result:
                ...
                change = (
                    f"+{row['change_from_prev']:.1f}"
                    if row["change_from_prev"] and row["change_from_prev"] > 0
                    else str(row["change_from_prev"]) if row["change_from_prev"] else "N/A"
                )
                print(
                    f"      {ts}: {row['value']:.1f}% (MA: {row['moving_avg_3hr']:.1f}%, Change: {change})"
                )

            print("\n💡 Time series features:")
            print("   - Moving averages (sliding window)")
            print("   - Lag/Lead for trend analysis")
...
"""
            ...
            minio.put_object(
                "duckdb-data",
                "raw/sales.csv",
                io.BytesIO(raw_data.encode()),
                len(raw_data.encode()),
            )
            print("      ✅ Raw data stored in MinIO")

            # Step 2: Import to hot storage (DuckDB)
            print("\n   2️⃣  LOAD: MinIO → DuckDB hot storage")
            duckdb.import_from_minio(
                database_id, "raw_sales", "duckdb-data", "raw/sales.csv", file_format="csv"
            )
            ...
            for row in results:
                ...
                total_revenue = (
                    ...
                    if isinstance(row["total_revenue"], str)
                    else row["total_revenue"]
                )
                num_sales = (
                    int(row["num_sales"]) if isinstance(row["num_sales"], str) else row["num_sales"]
                )
                print(
                    f"      {row['region']} - {row['product']}: ${total_revenue:.2f} "
                    + f"({num_sales} sales)"
                )

            print("\n💡 Data pipeline pattern:")
            print("   MinIO (raw) → DuckDB (hot) → Compute → MinIO (processed)")
