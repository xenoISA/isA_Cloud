...
def example_02_basic_log_pushing(host="localhost", port=50054):
    ...
    with LokiClient(host=host, port=port, user_id="example-user") as client:
        ...
        result = client.push_log(
            message="User authentication successful", labels=labels, timestamp=datetime.now()
        )
...
def example_03_simple_log_pushing(host="localhost", port=50054):
    ...
    with LokiClient(host=host, port=port, user_id="example-user") as client:
        ...
        client.push_simple_log(
            message="Debug: Processing request ID: 12345", service="api-gateway", level="DEBUG"
        )
        print("   ✅ DEBUG log")

        client.push_simple_log(
            message="User session created successfully", service="auth-service", level="INFO"
        )
        print("   ✅ INFO log")

        client.push_simple_log(
            message="High memory usage detected: 85%", service="monitoring", level="WARNING"
        )
        print("   ✅ WARNING log")
        ...
        print("   ✅ ERROR log with extra labels")
...
def example_05_query_logs(host="localhost", port=50054):
    ...
    with LokiClient(host=host, port=port, user_id="example-user") as client:
        ...
        for i in range(5):
            client.push_simple_log(
                message=f"API request {i}: /api/users", service="api-gateway", level="INFO"
            )
...
def example_06_label_management(host="localhost", port=50054):
    ...
    with LokiClient(host=host, port=port, user_id="example-user") as client:
        ...
        # Validate labels
        print("\n✅ Validating labels:")
        test_labels = {"app": "my-application", "environment": "production", "level": "info"}
...
def example_07_stream_management(host="localhost", port=50054):
    ...
    with LokiClient(host=host, port=port, user_id="example-user") as client:
        ...
        if streams and streams.get("total_count", 0) > 0:
            ...
            print("   No streams found yet (may need time for indexing)")
...
def example_08_monitoring_and_quota(host="localhost", port=50054):
    ...
    with LokiClient(host=host, port=port, user_id="example-user") as client:
        ...
        if quota:
            ...
            usage_percent = (
                (quota.get("today_used") / quota.get("daily_limit")) * 100
                if quota.get("daily_limit") > 0
                else 0
            )
            ...
            storage_percent = (
                (quota.get("storage_used_bytes") / quota.get("storage_limit_bytes")) * 100
                if quota.get("storage_limit_bytes") > 0
                else 0
            )
...
def example_12_microservices_logging(host="localhost", port=50054):
    ...
    with LokiClient(host=host, port=port, user_id="example-user") as client:
        ...
        services = [
            ...
            ("auth-service", "Authenticating user token"),
            ("order-service", "Fetching order 123 from database"),
            ("inventory-service", "Checking inventory for items"),
            ("payment-service", "Validating payment status"),
            ("api-gateway", "Response sent: 200 OK"),
        ]
...
def example_15_centralized_logging(host="localhost", port=50054):
    ...
    with LokiClient(host=host, port=port, user_id="example-user") as client:
        ...
        print("\n🔍 Query Pattern:")
        print("   To view auth-service logs:")
        print('   {service="auth-service"}')
        print("\n   To view all errors:")
        print('   {level="error"}')
        print("\n   To view specific cluster:")
        print('   {cluster="prod-us-west-1"}')
