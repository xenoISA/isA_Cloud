...
def example_10_sorted_set_operations(host="localhost", port=50055):
    ...
    with RedisClient(host=host, port=port, user_id="example-user") as client:
        ...
        leaderboard_data = {"Alice": 1500, "Bob": 2100, "Charlie": 1800, "Diana": 2500, "Eve": 1200}
...
def example_11_distributed_locks(host="localhost", port=50055):
    ...
    with RedisClient(host=host, port=port, user_id="example-user") as client:
        ...
        if lock_id:
            ...
            if renewed:
                print("   Lock renewed for another 30 seconds")
...
def example_12_pubsub_operations(host="localhost", port=50055):
    ...
    with RedisClient(host=host, port=port, user_id="example-user") as client:
        ...
        print("   Use: client.subscribe(['channel1', 'channel2'], callback_function)")
...
def example_17_real_world_cache(host="localhost", port=50055):
    ...
    with RedisClient(host=host, port=port, user_id="example-user") as client:
        ...
        if cached_response:
            ...
            print("   ✅ Stored in cache (TTL: 300s)")
...
def main():
    ...
    parser.add_argument(
        "--no-consul", action="store_true", help="Skip Consul discovery, use localhost directly"
    )
    parser.add_argument(
        "--example",
        type=int,
