...
# 重要: isA 平台内部服务间通信必须使用 user_id='isa_event'
with NATSClient(host='localhost', port=50056, user_id='isa_event') as client:
...
## ⚠️ Important: Multi-Tenant Stream Isolation (`user_id`)

NATS gRPC Gateway 实现了**多租户隔离**功能：不同的 `user_id` 会访问不同的 streams。

### 关键规则

**对于 isA 平台内部的微服务间事件通信，必须使用统一的 `user_id="isa_event"`：**

```python
# ✅ 正确 - 所有服务使用相同的 user_id
client = AsyncNATSClient(host='localhost', port=50056, user_id='isa_event')

# ❌ 错误 - 不同服务使用不同 user_id 会导致无法互相收发事件
client = AsyncNATSClient(host='localhost', port=50056, user_id='account-service')
client = AsyncNATSClient(host='localhost', port=50056, user_id='notification-service')
```

### 原理说明

1. **Stream 命名隔离**: gRPC Gateway 内部会按 `user_id` 给 stream 名称加前缀
   - `user_id="tenant1"` 访问的 `user-stream` 实际是 `tenant1-user-stream`
   - `user_id="tenant2"` 访问的 `user-stream` 实际是 `tenant2-user-stream`

2. **服务间通信需求**: account_service 发布 `user.created`，wallet/billing/notification 都要收到
   - 如果各服务用不同 `user_id`，它们访问的是不同的 streams
   - 结果：服务 A 发布的事件，服务 B 永远收不到

3. **解决方案**: 所有服务统一使用 `user_id="isa_event"`
   - 确保 Python 微服务和 Go 服务 (isA_Model) 访问同一套 streams
   - 实际业务用户 ID 保存在 `event.data` 中，不受此影响

### 使用场景

| 场景 | user_id | 说明 |
|------|---------|------|
| isA 微服务间事件通信 | `isa_event` | account, device, wallet, notification 等服务间通信 |
| 多租户 SaaS 应用 | `tenant_xxx` | 每个租户有独立的 stream 空间 |
| 独立测试环境 | `test_xxx` | 测试数据与生产数据隔离 |

### 代码示例 (isA_user 微服务)

```python
# core/nats_client.py - NATSEventBus
self._client = AsyncNATSClient(
    host=self.host,
    port=self.port,
    user_id="isa_event",  # 统一 user_id，所有服务共享 streams
...
class NotificationService:
    def __init__(self):
        # isA 平台服务间通信使用统一的 user_id='isa_event'
        self.nats = NATSClient(user_id='isa_event')
        self.running = False
