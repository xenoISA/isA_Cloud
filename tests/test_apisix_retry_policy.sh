...
#
# Validates:
# - Retries present in main route upstream (production + staging)
# - Retries present in health route upstream (production + staging)
# - Retry count is 2
# - Retry timeout is 6s
...
check_retry_config() {
    ...
    # Check retry_timeout field exists
    ...
    # Check retries appear in BOTH upstream blocks (main route + health route)
    local retries_count
